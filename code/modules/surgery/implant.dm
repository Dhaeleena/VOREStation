...
/datum/surgery_step/cavity/place_item/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	tool.loc = affected
	//if(istype(tool,/obj/item/device/nif)){var/obj/item/device/nif/N = tool;N.implant(target)} //VOREStation Add - NIF support //Removed NIF support! -Jon
	affected.cavity = 0
		...
		if (prob(find_prob))
			...
			if(istype(obj,/mob/living/simple_mob/animal/borer))
				...
				if(istype(obj,/obj/item/weapon/implant))
					...
					imp.imp_in = null
					imp.implanted = 0
				//else if(istype(tool,/obj/item/device/nif)){var/obj/item/device/nif/N = tool;N.unimplant(target)} //VOREStation Add - NIF support //Removed NIF support! -Jon
		else
